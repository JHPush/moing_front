...
const InviteMoim = ({gatheringId}) => {
  ...
  useEffect(() => {
    const fetchInviteData = async () => {
        ...
        const response = await axios.get(
            "https://ardbyd7sf7.execute-api.ap-northeast-2.amazonaws.com/dev/moing/detail/invitation"
        );

        // Lambda의 응답 구조에서 body가 JSON 객체인 경우
  ...
  const handleEmailInput = async () => {
    ...
    try {
      const response = await axios.post('https://ardbyd7sf7.execute-api.ap-northeast-2.amazonaws.com/dev/moing/detail/invitation', {
        email: email,
...
<div className="p-4 border rounded w-full max-w-md">
      ...
      {emailSent && <p className="text-green-600 mt-2">초대 이메일이 전송되었습니다!</p>}
    </div>
  </div>
